# metabolic-health-agent Template Setup
...
```
bunx degit epicweb-dev/epicflare my-metabolic-health-agent-app
cd my-metabolic-health-agent-app
```
...
- You can write to files in the repository (the script updates config files and
  replaces template `metabolic-health-agent` tokens across text files).
